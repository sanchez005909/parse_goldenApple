...
from data_for_request_products import (cookies, headers, json_data,
                                       url_card_product, url_products, params)
...
def gather_data():
    ...
    for page in tqdm.tqdm(range(2, pages+1)):
        products += get_page_data(page)['data']['products']
...
def get_data_product(lst_products: list):
    """
    ...
    for product in tqdm.tqdm(lst_products):
        card = get_product_card(product['itemId'])
        # print(card)
        item = {
